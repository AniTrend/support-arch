...
import io.wax911.support.getNavigationBarHeight

class SupportRefreshLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) : ViewGroup(context, attrs), NestedScrollingParent, NestedScrollingChild {
    ...
    private var mPermitRefresh = true
    private var mPermitLoad = true
    ...
    private var mCircleViews: Array<SupportCircleImage>? = null
    ...
    /* notify */ var isRefreshing: Boolean
        ...
        set(refreshing) {
            ...
            if (refreshing) {
                ...
                mRefreshing = true
                setTargetOffsetTopAndBottom(DIRECTION_TOP, (mDragTriggerDistances[DIRECTION_TOP] - mDragOffsetDistance).toInt())
                mNotify = false
    ...
    var isLoading: Boolean
        ...
        set(loading) {
            ...
            if (loading) {
                ...
                mLoading = true
                setTargetOffsetTopAndBottom(DIRECTION_BOTTOM, (-mDragTriggerDistances[DIRECTION_BOTTOM] - mDragOffsetDistance).toInt())
                mNotify = false
    ...
    private fun createProgressView() {
        this.mCircleViews = arrayOf(SupportCircleImage(context, CIRCLE_BG_LIGHT, (CIRCLE_DIAMETER / 2).toFloat()), SupportCircleImage(context, CIRCLE_BG_LIGHT, (CIRCLE_DIAMETER / 2).toFloat()))
    ...
    private fun moveSpinner(dir: Int, dragDistance: Float) {
        ...
        val tensionSlingshotPercent = Math.max(0f, Math.min(extraOS, mDragTriggerDistances[dir] * 2) / mDragTriggerDistances[dir])
        ...
        val offset = (mDragTriggerDistances[dir] * dragPercent + extraMove).toInt() * if (dir == DIRECTION_TOP) 1 else -1
    ...
    private fun animateOffsetToCorrectPosition(dir: Int, from: Int, listener: Animation.AnimationListener?) {
        ...
        mCircleViews!![dir].startAnimation(
                if (dir == DIRECTION_TOP) mAnimateToTopCorrectPosition else mAnimateToBottomCorrectPosition)
    }
    ...
    /**
     * Configure the swipe refresh layout using the parent context
     */
    fun configureWidgetBehaviorWith(context: Context?) = when (context != null) {
        true -> {
           val navigationHeight = resources.getNavigationBarHeight() + 16f.dipToPx()
           setDragTriggerDistance(DIRECTION_BOTTOM, navigationHeight)
